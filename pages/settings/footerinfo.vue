...
const paginate = async (page) => {
    ...
    if (page === "&laquo; Previous") {
        pageNumber.value = ((web_settings.value.current_page-1) == 0 ? 1 : (web_settings.value.current_page-1)) ;
        // ((web_settings.value.current_page-1) == 0 ? 1 : (web_settings.value.current_page-1))
...
<div class="shadow-md bg-white w-full h-[calc(100vh-6rem)] overflow-hidden rounded-md">
    ...
    <div class="w-full flex justify-center">
        ...
        <div class="flex">
            <a
                v-for="(page, index) in web_settings?.links"
                :key="index"
                @click="paginate(page.label)"
                v-html="page.label"
                :class="{'border-t-4 px-2 border-red-500 text-red-500': page.active}"
                class="px-2 pt-2 mt-1 block text-black"
                href="#"
            />
        </div>
        </div>
